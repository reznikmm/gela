...
package body Gela.Scanners is
   ...
   package Tables is
      ...
      function Rule (S : State) return Gela.Scanner_Types.Rule_Index;
   end Tables;
   ...
   procedure On_Accept
     (Self    : not null access Gela.Scanner_Handlers.Handler'Class;
   ...
   procedure Get_Token (Self : access Scanner'Class; Result : out Token) is
      ...
      End_Of_Buffer : constant Wide_Wide_Character :=
        Wide_Wide_Character'Val (Scanners.End_Of_Buffer);

      Current_State : State := Self.Start;
      Char          : Character_Class;
      Next_Rule     : Gela.Scanner_Types.Rule_Index;
      Skip          : Boolean := True;
   begin
      loop
         if Self.Buffer (Self.Next) = EOF then
            Result := Gela.Lexical_Types.End_Of_Input;
            return;
         end if;
   ...
   procedure Set_Handler
     (Self    : in out Scanner'Class;
      Handler : not null Gela.Scanner_Handlers.Handler_Access) is
   begin
