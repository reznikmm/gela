...
private
   ...
   subtype Context_Index  is Asis.ASIS_Natural range 0 .. 16;
   subtype Element_Index  is Asis.ASIS_Natural;
   subtype Unit_Index     is Asis.ASIS_Natural;
   subtype Property_Value is Asis.ASIS_Natural;

   type Context is record
      Index : Context_Index := 0;
   end record;

   Nil_Context : constant Context := (Index => 0);

   type Compilation_Unit is record
      Index : Unit_Index := 0;
   end record;

   function Assigned (Unit : Compilation_Unit) return Boolean;

   Nil_Compilation_Unit      : constant Compilation_Unit := (Index => 0);
   Nil_Compilation_Unit_List : constant Compilation_Unit_List  (1 .. 0) :=
       (1 .. 0 => (Index => 0));

   type Element is record
      Unit  : Compilation_Unit := (Index => 0);
      Index : Element_Index := 0;
   end record;

   function Assigned (Item : Element) return Boolean;

   Nil_Element      : constant Element := ((Index => 0), 0);
   Nil_Element_List : constant Element_List (1 .. 0) :=
       (1 .. 0 => ((Index => 0), 0));

   function Assigned (Element : Element_Index) return Boolean;
