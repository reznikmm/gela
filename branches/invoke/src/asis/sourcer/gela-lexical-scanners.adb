...
package body Gela.Lexical.Scanners is
   ...
   package Tables is
      ...
      function Rule (S : State) return Gela.Lexical.Types.Rule_Index;
   end Tables;
   ...
   procedure Get_Token (Self : access Scanner'Class; Result : out Token) is
      ...
      Next_Rule : Gela.Lexical.Types.Rule_Index;
      Skip      : Boolean := True;
   begin
      loop
         if Self.Buffer (Self.Next) = EOF then
            Result := Gela.Lexical.Tokens.End_Of_Input;
            return;
         end if;
         ...
         if Self.Rule = 0 then
            Self.Next := Self.To + 1;
            Result := Gela.Lexical.Tokens.Error;
            return;
         else
            Self.Next := Self.To;
            Next;
   ...
   procedure Set_Handler
     (Self    : in out Scanner'Class;
      Handler : not null Gela.Lexical.Handlers.Handler_Access) is
   begin
